...
:menu
	cls
	echo "	 _    _ _      ______       _   _              	  ___________		   "
	echo "	| |  | (_)     | ___ \     | | | |             	 |     |     |		   "
	...
	echo "3)Disable a user		4)Change all passwords (option)"
	echo "5)Disable guest/admin		6)Set password policy
	echo "7)Set lockout policy		8)Enable Firewall"
	echo "9)Search for media files (not working)	10)Disable services
	echo "11)UAC (not working)		12)Remote Desktop Config
...
:passwd
	echo Type the following into PowerShell:
	echo -----
	echo "Get-LocalUser | ForEach-Object {Set-LocalUser $_.Name -Password (ConvertTo-SecureString -AsPlainText "Cyb3rP@Tr10t" -Force)}"
	endlocal
	setlocal enabledelayedexpansion	
	pause
	goto :menu
...
:remDesk
	...
	if /I "%answer%"=="y" (
		...
		echo RemoteDesktop has been enabled, select "Reboot" for this to take full effect.
	)
	if /I "%answer%"=="n" (
		reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server" /v fDenyTSConnections /t REG_DWORD /d 1 /f
		echo RemoteDesktop has been disabled, select "Reboot" for this to take full effect.
	)
