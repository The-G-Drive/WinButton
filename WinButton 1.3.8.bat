...
if %errorlevel%==0 (
    ...
    echo Failure, no rights. Run as an Admin.
	pause
    exit
)
...
:terms	
	...
	cls
	echo 						       		  -----TERMS OF AGREEMENT-----
	echo -----------------------------------------------------------------------------------------------------------------------------------------------------
	echo 	By running this batch script, I, %username%, am fully responsible for any harm or damage that may be done to this comupter by my actions. 
	echo 			      This program is not created to cause harm, and I, %username% will not use it to cause harm. 
	echo 				       And if any harm is done to this computer, I, %username%, am fully responsible.
	echo -----------------------------------------------------------------------------------------------------------------------------------------------------
	set /p answer=Do you accpet these terms? [y/n]
		if /I {%answer%}=={y} (
			goto :menu
		) 
		if /I {%answer%}=={n} (
			echo Then you are unworthy.
			echo Committing Seppuku...
			del "%~f0"
			pause
			exit
		) else (
...
:userProp
	echo Setting up the password to never expire...
...
:passwd
	powershell -Command "& {Get-LocalUser | ForEach-Object {Set-LocalUser $_.Name -Password (ConvertTo-SecureString -AsPlainText "Cyb3rP@Tr10t" -Force)}}"
	echo The password for ALL user accounts is "Cyb3rP@Tr10t"
	endlocal
	setlocal enabledelayedexpansion	
	pause
	goto :menu
...
:remDesk
	...
	set /p answer=Is remote desktop required by the scenario??[y/n]
	if /I "%answer%"=="y" (
...
:secOpt
	...
	echo Warning: Make sure to write down the name of your current user, as this will remove the user selection bar at the corner of the screen.
	pause
